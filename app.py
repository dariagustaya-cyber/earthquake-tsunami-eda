...
def best_threshold(y_true, y_proba, mode="f1"):
    ...
    if mode == "youden":
        ...
        idx = int(np.argmax(j))
        # roc_curve includes an extra threshold at the start; clip to safe range
...
# Safe geophysical features (ignore metadata like 'nst'/'year' if present)
...
# Only let users control split/seed (keeps comparisons stable)
...
# -----------------------------------------------------------------------------
# 4) Train ONLY Neural Network (MLP) — auto best threshold
# -----------------------------------------------------------------------------
...
# Tuned-but-safe MLP (scales well and converges reliably)
mlp = MLPClassifier(
    hidden_layer_sizes=(64, 32, 16),
    activation="relu",
    alpha=1e-4,                 # mild L2 to reduce overfit
    early_stopping=True,        # use internal validation for generalization
    n_iter_no_change=20,
    max_iter=1200,
...
if st.button("Train MLP", type="primary", key="train_button"):
    ...
    # Save to session for prediction & download
    ...
    st.caption(f"Operating threshold selected by **{criterion_label}**: t* = **{t_star:.2f}**")
