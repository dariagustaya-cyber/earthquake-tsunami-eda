...
if dist_cols:
    ...
    for i, c in enumerate(dist_cols):
        ...
        dcols[i].plotly_chart(fig, use_container_width=True)
...
if algo == "Logistic Regression":
    ...
    clf = MLPClassifier(hidden_layer_sizes=(16,8), activation="relu", solver="adam", max_iter=500, random_state=random_state)
...
if st.button("Train model", type="primary"):
    ...
    t = st.slider("Decision threshold", 0.05, 0.95, 0.50, 0.01)
    ...
    st.plotly_chart(px.imshow(cm_t, text_auto=True, x=["Pred 0","Pred 1"], y=["True 0","True 1"],
                   color_continuous_scale="Blues", title=f"Confusion Matrix (th = {t:.2f})"), use_container_width=True)
